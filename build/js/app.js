var GoToTrip = function() {
	...
	this.createBookmarksLink = function(el) {
		...
		parent = $(parent);
		if (!parent.attr('data-href')) parent.attr('data-href', window.location.href);
   ...
   * */
	this.createSliders = function() {
		...
		gtSliders.each(function() {
			...
			$(window).resize(function() {
				findScreenConst();
				setBaseWidth();
				if (currPosition > innerCount - screenConst) currPosition = innerCount - screenConst;
	...
	this.currTime = function() {
		$('.gt-curr-time').each(function() {
			...
			month = monthes[month];
			if ($('.gt-curr-time-time')) $('.gt-curr-time-time').html(hours + ':' + minutes);
			if ($('.gt-curr-time-date')) $('.gt-curr-time-date').html(weekday + ', ' + day + ' ' + month + ' ' + year);
		});
	};
